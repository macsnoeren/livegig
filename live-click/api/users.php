...
function getUsersWithBands(PDO $db): array {
    $users = $db->query('SELECT id, username, email, role, created_at FROM users ORDER BY username')->fetchAll();
    foreach ($users as &$u) {
...
if ($method === 'POST') {
    ...
    if ($id) {
        if ($password) {
            $db->prepare('UPDATE users SET username=?,email=?,password_hash=?,role=? WHERE id=?')
               ->execute([$username,$email,password_hash($password,PASSWORD_DEFAULT),$role,$id]);
        } else {
